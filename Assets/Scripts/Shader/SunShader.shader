Shader "Custom/SunShader"
{
    Properties
    {
        [HDR] _EmissionColorA("ColorA", Color) = (0,0,0,0)
        [HDR] _EmissionColorB("ColorB", Color) = (0,0,0,0)
        [HDR] _EmissionColorC("ColorC", Color) = (0,0,0,0)
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                float Speed = _Speed / 10000;
                float NoiseA = snoise((IN.positionOS.xyz + _Time.y * Speed) * _LowScale);
                float NoiseB = snoise((IN.positionOS.xyz + _Time.y * Speed + 5) * _HighScale);
                NoiseA = NoiseA / 2.0 + 0.5;
                NoiseA = pow(NoiseA, _LowOverlay);
                NoiseB = NoiseB / 2.0 + 0.5;
                NoiseB = pow(NoiseB, _HighOverlay);
                float4 Color = lerp(_EmissionColorA, _EmissionColorB, NoiseA);
                Color = lerp(Color, _EmissionColorC, NoiseB);
                return Color;
            }
            ENDHLSL
        }
